...
def main():
    ...
    decodedMessage = hackVigenere(ciphertext)
...
def attemptHackWithKeyLength(ciphertext, mostLikelyKeyLength):
    ...
    for indexes in itertools.product(range(NUM_MOST_FREQ_LETTERS), repeat=mostLikelyKeyLength):
        ...
        if detectEnglish.isEnglish(decryptedText):
            ...
            print "   Key %s: %s" % (possibleKey, decryptedText[:200])
...
def hackVigenere(ciphertext):
    ...
    if not SILENT_MODE:
        ...
        print '\nKasiski Examination results say the most likely key lengths are: ' + keyLengthStr + '\n'
    hackedMessage = None
    for keyLength in allLikelyKeyLengths:
        if not SILENT_MODE:
            print 'Attempting decrypt with key length %s (%s possible keys)...' % (keyLength, NUM_MOST_FREQ_LETTERS ** keyLength)
